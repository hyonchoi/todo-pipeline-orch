...
- **Bootstrap once per repo**: create the label vocabulary before using the form — GitHub silently drops form labels that do not exist on the repo. Existing labels are left untouched, including color/description. Run `tpo todos labels sync <project>` (to be added); today: `uv run python -c "from pathlib import Path; from hermes_pipeline.github_issues import ensure_labels; print(ensure_labels(Path('.')))"`.
