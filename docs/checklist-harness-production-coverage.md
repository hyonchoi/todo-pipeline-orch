...
|---|---------|-----------|------------|------------------------|------------------|-----------|
| 1 | Cycle starts, kanban mode selected | — | phases registered as kanban tasks | `kanban_tasks.prepare_todo_phases` + `kanban_tasks.create_prepared_todo_phases` | Each phase in `phases.yaml` has a corresponding kanban task; parent-chain (`--parent`) links each phase to its predecessor in phase order; gate phases have no worker goal and receive a sticky `needs_input` block; non-gate phases are created with `--goal` | `tests/test_kanban_tasks.py::TestPrepareAndCreateTodoPhases::test_creates_tasks_with_parent_chain`, `::test_gate_phase_registered_with_sticky_block_without_goal` |
| 2 | Registration succeeds | tasks created | expected-phase manifest persisted | `kanban_tasks._persist_expected_phases` (internal to `create_prepared_todo_phases`) | `all_phases_complete`'s completeness check (row 9) can find the persisted phase-key list for this tick | `tests/test_kanban_tasks.py::TestPersistExpectedPhases::test_writes_to_project_hermes_dir` |
| 3 | Registration fails mid-way (any `hermes kanban create` non-zero exit) | tasks partially created | tasks archived, exception raised | `kanban_tasks._archive_tasks` | Previously-created task IDs for this tick no longer appear in a subsequent `get_todo_kanban_tasks` query (archived, not active); `RuntimeError` propagates to caller | `tests/test_kanban_tasks.py::test_sentinel_failure_retains_child_first_cleanup`, `::test_durable_create_failure_does_not_expose_hermes_stderr` |
