...
|---|---------|-----------|------------|------------------------|------------------|-----------|
| 1 | Cycle starts, kanban mode selected | — | phases registered as kanban tasks | `kanban_tasks.register_todo_phases` | Each phase in `phases.yaml` has a corresponding kanban task; parent-chain (`--parent`) links executable tasks to their predecessor in phase order; gate phases are detached and created with `--initial-status BLOCKED`, non-gate phases created with `--goal` | `tests/test_kanban_tasks.py::TestRegisterTodoPhases::test_creates_tasks_with_parent_chain`, `::test_gate_phase_registered_blocked_without_goal` |
