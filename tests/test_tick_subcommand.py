...
# Moved from test_tick_subcommand_edge.py: additional coverage for tick subcommand edge cases and codepaths.
...
# Moved from test_scan_loop.py: tests for scan loop behavior (multi-project iteration).

class TestScanLoop:
    """Tests for tick's multi-project scan behavior."""

    # Overrides the module autouse fixture of the same name: these tests serve
    # issue #1 ("First task") instead of #10.
    @pytest.fixture(autouse=True)
    def _github_todo_10(self, github_todo):
        return github_todo(1, "First task")

    def test_tick_skips_disabled_projects(self, tmp_path, mocker):
        """tick should skip projects with enabled=false in project.toml."""
        projects_dir = tmp_path / "projects"
        projects_dir.mkdir()

        _create_project(projects_dir, "project-a")
        pb = _create_project(projects_dir, "project-b")
        pb_hermes = pb / ".hermes"
        ...
        mocker.patch("hermes_pipeline.cli.run_selection", mock_selection)
        exit_code = _cmd_tick(FakeArgs(), config)

        assert exit_code == 0
        assert "project-a" in selection_calls
        assert "project-b" not in selection_calls

    def test_tick_never_copies_global_state_into_a_project(self, tmp_path, mocker):
        """Hard cutover: the legacy global-state migration is gone."""
        projects_dir = tmp_path / "projects"
        projects_dir.mkdir()
        pa = _create_project(projects_dir, "project-a")
        state_dir = tmp_path / "state"
        state_dir.mkdir()
        (state_dir / "current_tick_id.txt").write_text("old-tick-123\n")
        config = Config(projects_dir=projects_dir, state_dir=state_dir)

        mocker.patch("hermes_pipeline.cli.run_selection",
                     side_effect=lambda **kwargs: make_decision(rationale="test rationale"))
        _cmd_tick(FakeArgs(), config)
