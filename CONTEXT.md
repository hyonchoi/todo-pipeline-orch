...
**Plan**:
Implementation-ready execution steps for a TODO, carried either in the issue's folded `Implementation Plan` block or by a legacy `Plan:` repository-relative path. New TODOs created with `tpo todos create` embed the Plan and its `json tpo-plan` manifest in the issue body. It is the execution authority and makes the TODO pipeline-actionable; design explorations, reviews, specifications, references, and test plans do not qualify unless they explicitly define executable implementation steps.
...
**Document Attachment**:
A user-confirmed association between a TODO and an existing repository document in the role of Plan, Spec, or Reference. A Plan may instead be embedded directly in the issue body, which is the default for `tpo todos create`. One document may serve as both Plan and Spec when it genuinely contains both execution instructions and the authoritative outcome contract, but it is not duplicated as a Reference. Attachments may be proposed during TODO creation or revision, but are never made silently and never replace an existing attachment without explicit approval.
